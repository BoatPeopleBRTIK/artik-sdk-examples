...
static artik_error test_websocket_read(int timeout_ms, bool enable_sdr)
{
	...
	ret = cloud->websocket_set_connection_callback(handle, websocket_connection_callback, &handle);
	if (ret != S_OK) {
		fprintf(stderr, "TEST failed, could not open Websocket (%d)\n", ret);
		return ret;
	}
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "t:d:m:s")) != -1) {
		...
		case 's':
			...
			break;
		default:
			printf("Usage: websocket-cloud-test [-t <access token>] [-d <device id>] [-m <JSON type test message>] [-s for enabling SDR (Secure Device Registered) test]\r\n");
			return 0;
		}
	}
